...
def tensor(sig: np.ndarray):
    try:
        return tch.tensor(sig)
    except:
        return tch.tensor(sig.copy())

def detect_pitch_frequency(sig: np.ndarray, fs, frame_time, freq_high):
    sig = tensor(sig)
...
class PitchAngularVelocityController(ComponentInterface):
    ...
    def step(self):
        ...
        pitch_frequency = detect_pitch_frequency(
            sig, self.microphone.sample_rate,
            frame_time=0.3, freq_high=700
        )
    ...
    def entry(cls, speed=100, **kwargs):
        ...
        """
        # 50ms
        # tafn.detect_pitch_frequency takes 0.25 or 0.5 second per frame
        # pass 1 second to the function and take the last 
